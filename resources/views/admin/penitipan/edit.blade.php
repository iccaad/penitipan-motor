...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-[#1e40af] px-6 py-4">
            ...
            <p class="text-blue-200 text-sm mt-1">Hanya lakukan perubahan jika terdapat kesalahan input dari pengguna.</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                            name="nama_penitip" 
                            value="{{ old('nama_penitip', $item->nama_penitip) }}" 
                            required 
                            @class([
                    ...
                    <div>
                        ...
                        <input type="text" name="no_hp" value="{{ old('no_hp', $item->no_hp) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nomor Polisi <span class="text-red-500">*</span></label>
                        <input type="text" name="nomor_polisi" value="{{ old('nomor_polisi', $item->nomor_polisi) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all uppercase font-bold text-[#1e40af]">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Merk</label>
                            <input type="text" name="merk_motor" value="{{ old('merk_motor', $item->merk_motor) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Tipe</label>
                            <input type="text" name="tipe_motor" value="{{ old('tipe_motor', $item->tipe_motor) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="cc_motor" value="{{ old('cc_motor', $item->cc_motor) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Warna Motor</label>
                            <input type="text" name="warna_motor" value="{{ old('warna_motor', $item->warna_motor) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Tanggal Rencana Ambil</label>
                        <input type="date" name="tanggal_rencana_ambil" value="{{ old('tanggal_rencana_ambil', $item->tanggal_rencana_ambil) }}" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#1e40af] outline-none transition-all">
                    </div>
                </div>
            </div>

            <div class="mt-8 pt-5 border-t border-gray-200 flex justify-end space-x-3">
                <a href="{{ route('admin.penitipan.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none transition-colors">
                    Batalkan
                </a>
                <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-[#1e40af] rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 transition-colors shadow-md flex items-center">
