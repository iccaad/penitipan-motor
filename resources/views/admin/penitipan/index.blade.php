                        ...
                        <tr class="hover:bg-blue-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.penitipan.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini secara permanen?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-gray-600 hover:text-[#ef4444] hover:bg-red-100 rounded-lg transition-colors" title="Hapus Data">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </form>

                                    @if($item->status == 0)
                                        <form action="{{ route('admin.penitipan.ambil', $item->id) }}" method="POST" class="inline ml-2 border-l pl-2 border-gray-300">
                                            @csrf
                                            <button type="submit" onclick="return confirm('Verifikasi bahwa kendaraan telah diambil oleh pemilik?');" class="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors shadow-sm">
                                                Verifikasi Ambil
                                            </button>
                                        </form>
                                    @endif
